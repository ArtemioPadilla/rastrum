...
export const reportDismissHandler: ActionHandler<Payload> = {
  op: 'report_dismissed',
  ...
  async execute(admin, payload, _actor, _reason) {
    ...
    if (!before) throw new Error('report.dismiss: target not found');
    const { error } = await admin.from('reports').update({ status: 'dismissed' }).eq('id', payload.report_id);
    if (error) throw new Error(`report.dismiss: ${error.message}`);
