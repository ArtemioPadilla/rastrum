...
export const commentLockHandler: ActionHandler<Payload> = {
  op: 'comment_lock',
  ...
  async execute(admin, payload, _actor, _reason) {
    ...
    if (!before) throw new Error('comment.lock: target not found');
    const { error } = await admin.from('observation_comments').update({ locked: true }).eq('id', payload.comment_id);
    if (error) throw new Error(`comment.lock: ${error.message}`);
