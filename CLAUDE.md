...
| Edge Function 403 from cron after rotation | `CRON_SECRET` mismatch — Vault and GitHub Secret out of sync | Rotate Vault **first**, then GitHub Secret, then re-run `db-apply`. See `docs/runbooks/cron-secret-rotation.md` |
| Cron EF returns 403 from `community-backfill.yml` | Workflow doesn't send `X-Cron-Secret` header | Pass `-H "X-Cron-Secret: ${{ secrets.CRON_SECRET }}"` to every curl call targeting a cron-only EF |
...
| Symptom | Cause | Fix |
|---|---|---|
| Required branch-protection check expects a status that's never reported on the PR | `db-validate.yml` had `on: pull_request: paths: [supabase-schema.sql, …]` so the workflow only fired for schema PRs. Combined with `validate` being a required check, any PR not touching those paths became unmergeable — the rule waits for a status that never comes, including via `--admin` override (which still requires the check to be *present*, not just passing). | Remove the `paths:` filter so the workflow runs on every PR. Cost is ~1.5 min CI on each PR; the idempotent schema apply is also a useful smoke test for any change. Done in #1015. **Alternative anti-pattern:** relaxing the rule to "required when present" — hides the same misconfiguration if a future PR re-adds a path filter. |
| 6+ weeks of accumulated schema bugs ship silently from rushed merges | The validate gate was broken at line 1326 (`storage.foldername()` missing in CI Postgres bootstrap). For every PR after that, schema apply errored before validating anything. PRs #550, #803, #914, #995, #997, #1002 each shipped schema bugs that would have been caught: missing `);` in CREATE TABLE, missing `DROP POLICY IF EXISTS` guards, `taxon_id` vs `primary_taxon_id` column-name drift, nested `$$ … $$` dollar-quotes, missing `WITH (security_invoker = true)`, missing `SET search_path` on `SECURITY DEFINER` functions, broken `karma_events.reason` CHECK constraint that silently rejected `streak_freeze_consumed`/`pool_donation`/`ai_sponsorship_*` writes from PR #997's lottery feature redefinition. | Restore the gate AND peel the cascading bugs in one coherent PR (see next row). Going forward: if `db-validate` ever reports "skipped" on a PR that *should* trigger it, treat as a P0 incident — the gate may be silently broken. |
| Multiple PRs that share a CI gate, each splitting off one author's bugfix, deadlock each other because none can pass alone | Splitting cascading fixes into per-author PRs (#1006–#1013, #1018+#1019+#999) creates a circular dependency: each PR's CI tries to apply the whole schema (or run the whole test suite), so it fails on the *other* PRs' still-in-main bugs. None can be merged first because none can be green first. | When fixes are **independent in code** but **coupled in CI**, they are one concern. Bundle them into a single PR with **atomic commits**, one per bug, each commit's message naming the originating PR (e.g. `fix(schema): mv_recent_species selects primary_taxon_id (not taxon_id)\n\nIntroduced in #1002.`). GitHub's "Files changed → Show diffs for individual commits" preserves per-commit reviewability; the merge is atomic and revertable as one unit. This is the Pattern B (single PR, atomic commits) shape used by Linux kernel, git itself, and most OSS — **not** "one PR per author" which only applies when fixes are independent in CI too. |
