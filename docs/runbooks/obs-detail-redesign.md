...
| #124 | merged 2026-04-29 | PR5 — Location tab (read-only `MapPicker mode='view'` + edit-modal `MapPicker mode='edit'` with `pickerId='obs-detail-edit'`; `wireManagePanelLocation` listens for `rastrum:mappicker-save` and UPDATEs `observations.location` as `SRID=4326;POINT(lng lat)`) |
