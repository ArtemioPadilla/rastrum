...
1. **Bbox-aware species cascade** — ✅ partial. `IdentifyInput.mediaCrop?: { bbox, source }` shipped (`src/lib/identifiers/types.ts`). MegaDetector emits the bbox in source-image space (`unletterbox()` applied). Cascade engine forwards it as `mediaCrop` to subsequent plugins (`src/lib/identifiers/cascade.ts`). Phi-vision honors it: pre-crops the dataUrl via `cropDataUrlToBbox()` (`src/lib/identifiers/bbox-crop.ts`) before running local inference. **Server-side path still TBD** — Claude and PlantNet plugins are thin proxies to the `identify` Edge Function; for them to honor `mediaCrop` we need either a `crop_bbox` body field on the Deno function (preferred) or a client-side fetch+crop+R2-upload step. Track under `megadetector-bbox-cascade-server` once started.
...
4. **Hosting recipe** — ✅ shipped at [`infra/megadetector/`](../../../infra/megadetector/). One-shot `convert.sh` does YOLOv5 export + INT8 quantise; README documents R2 upload + CI env var. Modal/Lambda/Replicate variants for the server-side fallback path are still TBD.
